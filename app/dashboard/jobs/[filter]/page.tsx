...
export default function JobsPage() {
  ...
  const getStatusTagBorderColor = (tag: any) => {
    switch (tag) {
      case 'Matched':
        // Apply filter effect to enhance the green border color
        return '#2dd179'; // Enhanced green that mimics the filter effect
      case 'Remote':
        return '#60a5fa'; // More vibrant blue for remote
      case 'Awaiting Feedback':
        return '#3b82f6'; // Vibrant blue border matching the glow
      default:
        return '#6b7280';
    }
  };
            ...
            <div className="flex space-x-4 mb-6">
              {tabs.map((tab) => (
                <button
                  ...
                  disabled={isPending}
                  className={`px-4 py-2 rounded-full text-sm font-medium transition-all duration-300 ${isDarkMode ? 'metal-shadow' : 'metal-shadow-light'} cursor-pointer ${
                    activeTab === tab
                            ...
                            {job.important_tags.map((tag, index) => {
                              ...
                              return (
                                <div
                                  ...
                                  style={{ 
                                    backgroundColor: getStatusTagColor(tag),
                                    border: `0.5px solid ${getStatusTagBorderColor(tag)}`,
                                    boxShadow: tag === 'Matched' 
                                      ? `0 0 0 0.5px rgba(45, 209, 121, 0.8), 0 0 8px rgba(45, 209, 121, 0.4), 0 0 16px rgba(45, 209, 121, 0.2)`
                                      : tag === 'Remote'
                                      ? `0 0 0 0.5px rgba(96, 165, 250, 0.8), 0 0 8px rgba(96, 165, 250, 0.4), 0 0 16px rgba(96, 165, 250, 0.2)`
                                      : tag === 'Awaiting Feedback'
                                      ? `0 0 0 0.5px rgba(59, 130, 246, 0.8), 0 0 8px rgba(59, 130, 246, 0.4), 0 0 16px rgba(59, 130, 246, 0.2)`
                                      : 'none'
                                  }}
                                >
                                  {icon && (
                                    <img 
                                      ...
                                      style={{ 
                                        filter: tag === 'Matched' 
                                          ...
                                          : tag === 'Remote'
                                          ? 'brightness(0) saturate(100%) invert(59%) sepia(93%) saturate(1352%) hue-rotate(200deg) brightness(99%) contrast(96%)'
                                          : tag === 'Awaiting Feedback'
                                          ? 'brightness(0) saturate(100%) invert(58%) sepia(96%) saturate(1271%) hue-rotate(213deg) brightness(97%) contrast(94%)'
                                          : 'brightness(0) saturate(100%) invert(100%)'
                                      }}
                                    />
                                  )}
                          ...
                          <div className="flex items-center space-x-3">
                            ...
                            <button 
                              ...
                              style={{ 
                                ...
                                border: job.status === 'Apply' 
                                  ? '0.5px solid #4a90e2' 
                                  : '0.5px solid #2dd179',
                                boxShadow: job.status === 'Apply'
                                  ? '0 0 0 0.5px rgba(74, 144, 226, 0.6), 0 0 6px rgba(74, 144, 226, 0.3), 0 0 12px rgba(74, 144, 226, 0.15)'
                                  : '0 0 0 0.5px rgba(45, 209, 121, 0.8), 0 0 8px rgba(45, 209, 121, 0.4), 0 0 16px rgba(45, 209, 121, 0.2)'
                              }}
                            >
                              {job.status === 'Apply' && (
                                <img 
                                  ...
                                  alt="Edit" 
                                  className="w-4 h-4"
                                  style={{ filter: 'brightness(0) saturate(100%) invert(57%) sepia(73%) saturate(1204%) hue-rotate(200deg) brightness(94%) contrast(87%)' }}
                                />
                              )}
                              {job.status === 'Applied' && (
                                <img 
